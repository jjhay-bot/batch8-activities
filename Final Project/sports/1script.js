        ...
        for (let i = 0; i < target.length-1; i++) {
            ...
            .then(data => {
                let image = data.data.relationships.images.data[0].variants[0].thumbnail.url;
                    if(targetSample == 228) {
                        return groups.innerHTML += `<a class="thisLink" href="">
                                                <div class="groupDiv">${data.data.attributes.name}</div>
                                                <div class="description" onclick="getArray()" >Synchronised Swimming used to be known as "water ballet". This is because the routines are essentially a series of athletic movements choreographed to a piece of chosen music. Not unlike staged ballet, except that it is performed in the water. </div>
                    ...
                    if(targetSample == 506) {
                        return groups.innerHTML += `<a class="thisLink" href="">
                                                        <div class="groupDiv">${data.data.attributes.name}</div>
                                                        <div class="description" onclick="getArray()" >Aqua jogging, aka deep water running, is a form of cardiovascular exercise that mimics the motion of jogging while submerged in water. You can do aqua jogging by running laps in the pool or wearing a flotation device around your trunk and running in place.
                                                            <br><br>
                    ...
                    if(targetSample == 504) {
                        groups.innerHTML += `<a class="thisLink" href="">
                                                <div class="groupDiv">${data.data.attributes.name}</div>
